 ...
 * * this prototype.  One instance of Grid is created per Grid directive instance.
  ...
  Grid.prototype.buildColumns = function buildColumns() {
    ...
    if (!gridUtil.isNullOrUndefined(this.options.rowHeader)) {
      var col = new GridColumn({name: "rowHeader", displayName: '', enableColumnMenu: false, pinned: true}, 0, self);
    ...
    if (self.isExpandable) {
      ...
      expandableCol.cellTemplate = '<div class="ui-grid-cell uiGridExpandableButtonsCell"><div class="ui-grid-cell-contents"><button class="expandableButton" ng-if="!row.isExpanded;" ng-click="grid.api.expandable.toggleRowExpansion(row.entity)">E</button><button class="expandableButton" ng-if="row.isExpanded" ng-click="row.isExpanded = false">C</button></div></div>';
